...
export class CustomSelect extends FormElement {
  ...
  getDropdownItemHTML(option: CustomSelectOptionType) {
    return html`
      ...
      >
        <span class="group-focus:ring-u-focus block group-focus-visible:ring-2">
  ...
  renderOptionLabel(
    ...
    const imgUrl = _option.el.getAttribute('data-img-url')
    const imgAlt = _option.el.getAttribute('data-img-alt')
  ...
  get _currentToggleLabel() {
    ...
    const imgUrl = selectedOption.el.getAttribute('data-img-url')
    const imgAlt = selectedOption.el.getAttribute('data-img-alt')
    ...
    return html`
      <span class="flex items-center">
        <img
          ...
          height="16"
        />
        <span class="ms-xs">${selectedOption.label}</span>
      </span>
    `
  }
  ...
  get getCustomSelectHTML() {
    return html`
      ...
      >
        ...
        >
          <span class="pr-sm block truncate">${this._currentToggleLabel}</span>
...
const CLASSES_MAPPING = {
  ...
  LABEL:
    'sr-only absolute caption text-foreground-secondary duration-300 transform -translate-y-2.5 scale-75 top-3.5 start-sm-forms-padding lg:start-sm-forms-padding z-10 origin-[0] peer-focus:text-foreground peer-disabled:text-border peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-2.5 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto',
  HELP_TEXT:
    'caption text-foreground-secondary start-sm-forms-padding lg:start-sm-forms-padding',
  DROPDOWN:
