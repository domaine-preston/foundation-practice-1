...
>(
  ...
  abstract class WithShopifyCartClientClass extends superClass {
    connectedCallback(): void {
      ...
      const registerForCartUpdates = this.hasAttribute(
        'register-for-cart-updates'
      )
      if (
        !!sectionId &&
        typeof sectionId === 'string' &&
        registerForCartUpdates
      ) {
    ...
    private normalizeExistingDiscountCodes(cart: ShopifyCart): string[] {
      return [
        // Cart-level discount codes
        ...(cart.discount_codes?.map((d) => d.code).filter(Boolean) ?? []),
        // Cart-level discount applications
        ...(cart.cart_level_discount_applications
          ?.filter(
            (discount): discount is DiscountApplication =>
              !!discount && typeof discount === 'object' && 'title' in discount
          )
        ...
        ...(cart.items ?? []).flatMap(
          (item) =>
            item.line_level_discount_allocations
              ?.filter(
                (alloc): alloc is LineLevelDiscountAllocation =>
                  !!alloc &&
                  typeof alloc === 'object' &&
                  'discount_application' in alloc
              )
              .map((alloc) => alloc.discount_application.title) ?? []
        ),
      ].filter(Boolean)
    }

    private buildMergedDiscount(incoming?: string): string | undefined {
      if (incoming === undefined) return undefined
      const trimmed = incoming.trim()
      if (trimmed === '') return ''

      const existingDiscountCodes = this.normalizeExistingDiscountCodes(
        this.cart
      )

      if (existingDiscountCodes.length > 0 && !this.cart.discount_codes) {
        this.cart.discount_codes = existingDiscountCodes.map((code) => ({
          code,
          applicable: true,
        }))
      }

      const current = existingDiscountCodes
        .flatMap((c) => c.split(',').map((x: string) => x.trim()))
        .filter(Boolean)

      const lower = new Set(current.map((c) => c.toLowerCase()))
      if (!lower.has(trimmed.toLowerCase())) current.push(trimmed)
    ...
    async updateCartNoteAndAttributes(
      ...
      const discountCombined = this.buildMergedDiscount(update.discount)

      const body: Record<string, unknown> = {
        sections: this.sectionsToFetch,
        sections_url: window.location.pathname,
      }
      if (update.note !== undefined) body.note = update.note
      if (update.attributes) body.attributes = update.attributes
      if (discountCombined !== undefined) body.discount = discountCombined

      const cart = await this.client<ShopifyCart>('/update.js', { body })
      await this._handleCartChange(cart)
